...
filteredMovies = movies.filter(m => m.type== 'TV Series');

  // 2. Get all movies longer then 120 minutes
  filteredMovies = movies.filter(m => m.duration>120);

  // 3. Get all movies with genre "Animation"
  filteredMovies = movies.filter(m => m.genre=='Animation');

  // 4. Get all sci-fi movies with rating above 8.0
  filteredMovies = movies.filter(m => m.type=='sci-fi' && m.rating>8); 

  // 5. Get all movies made between 2010 and 2020
  filteredMovies = movies.filter(m => m.year>=2010 && m.year<=2020); 

  // 6. Find all Animation movies after 2015 with rating > 8.0
filteredMovies = movies.filter(m => m.type=='Animation' && m.year>2015 && m.rating>8.0);
